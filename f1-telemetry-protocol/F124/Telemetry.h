...
namespace F124
{
    ...
    struct PacketHeader
    {
        uint16 m_packetFormat;           // 0 2024
        uint8 m_gameYear;                // 2 Game year - last two digits e.g. 24
        uint8 m_gameMajorVersion;        // 3 Game major version - "X.00"
        uint8 m_gameMinorVersion;        // 4 Game minor version - "1.XX"
        uint8 m_packetVersion;           // 5 Version of this packet type
        uint8 m_packetId;                // 6 Identifier for the packet type
        uint64 m_sessionUID;             // 7 Unique identifier for the session
        float m_sessionTime;             // 15 Session timestamp
        uint32 m_frameIdentifier;        // 19 Identifier for the frame the data was retrieved on
        uint32 m_overallFrameIdentifier; // 23 Overall identifier for the frame the data was retrieved on, doesn't go back after flashbacks
        uint8 m_playerCarIndex;          // 27 Index of player's car in the array
        uint8 m_secondaryPlayerCarIndex; // 28 Index of secondary player's car in the array (splitscreen) - 255 if no second player
    };
    ...
    //-----------------------------------------------------------------------------
    // Participants - 1350 bytes
    //-----------------------------------------------------------------------------

    static const uint cs_maxParticipantNameLen = 48;
    ...
    struct CarSetupData
    {
        ...
        float m_rearLeftTyrePressure;   // Rear tyre pressure (PSI)
        float m_rearRightTyrePressure;  // Rear tyre pressure (PSI)
        float m_frontLeftTyrePressure;  // Front tyre pressure (PSI)
        float m_frontRightTyrePressure; // Front tyre pressure (PSI)
    ...
    struct CarStatusData
    {
        ...
        uint8 m_actualTyreCompound;      // F1 Modern - 16 = C5, 17 = C4, 18 = C3, 19 = C2, 20 = C1, 21 = C0, 7 = inter, 8 = wet
    ...
    //-----------------------------------------------------------------------------
    // Final Classification - 1020 bytes
    //-----------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    // Lobby Info - 1306 bytes
    //-----------------------------------------------------------------------------

    static const uint cs_maxLobbyInfoNameLen = 48;
    ...
    struct LobbyInfoData
    {
        ...
        uint8 m_nationality;                 // Nationality of the driver
        uint8 m_platform;                    // 1 = Steam, 3 = PlayStation, 4 = Xbox, 6 = Origin, 255 = unknown
        char m_name[cs_maxLobbyInfoNameLen]; // Name of participant in UTF-8 format – null terminated
                                             // Will be truncated with ... (U+2026) if too long
